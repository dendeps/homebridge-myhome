...
class MyHomeClient {
    ...
    constructor(_ipaddress, _port, _password, _classToCallback) {
        ...
        this.dimmerSetLevels = [2,3,4,5,6,7,8,9,10,10];
        this.buffers = {};
        this.lightBuses = [];
    ...
    dimmer10Command(_address,_bri) {
        ...
        if (_bri > 0) {
            this._bufferCommand("DIMMER",address,sprintf("*1*%s*%s##",dimmerSetLevels[Math.ceil(_bri/10) +1],address),500);
        } else {
            this._bufferCommand("DIMMER",address,sprintf("*1*0*%s##",address),500);
        }
    }
        ...
        frames.forEach(function(frame) {
            ...
            if (extract) {
                ...
                if (this.parent != null && this.parent.onMonitor != null && typeof(this.parent.onDimmer) == 'function')    
                    this.parent.onDimmer(address,this.dimmerLevels[level]);
                }
